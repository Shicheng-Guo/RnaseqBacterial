...
strain1<-read.table("strain.tab.matrix.freq.txt",head=T,sep="\t",row.names=1)
strain2<-read.table("strain.tab.matrix.counts.txt",head=T,sep="\t",row.names=1)
...
matrix1<-t(strain1[order(rowSums(strain1),decreasing = T)[1:300],])
head(matrix1)
...
write.csv(strain1,file="strain.tab.matrix.freq.csv",quote=F,row.names = F)
write.csv(strain2,file="strain.tab.matrix.counts.csv",quote=F,row.names = F)
